# frozen_string_literal: true

# stub: rspec-rails 8.0.1 ruby lib

Gem::Specification.new do |s|
  s.name = 'rspec-rails'
  s.version = '8.0.1'

  s.required_rubygems_version = Gem::Requirement.new('>= 0') if s.respond_to? :required_rubygems_version=
  if s.respond_to? :metadata=
    s.metadata = { 'bug_tracker_uri' => 'https://github.com/rspec/rspec-rails/issues',
                   'changelog_uri' => 'https://github.com/rspec/rspec-rails/blob/v8.0.1/Changelog.md', 'documentation_uri' => 'https://rspec.info/documentation/', 'mailing_list_uri' => 'https://groups.google.com/forum/#!forum/rspec', 'source_code_uri' => 'https://github.com/rspec/rspec-rails' }
  end
  s.require_paths = ['lib']
  s.authors = ['David Chelimsky', 'Andy Lindeman']
  s.cert_chain = ["-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n"]
  s.date = '1980-01-02'
  s.description = 'rspec-rails integrates the Rails testing helpers into RSpec.'
  s.email = 'rspec@googlegroups.com'
  s.homepage = 'https://github.com/rspec/rspec-rails'
  s.licenses = ['MIT']
  s.rdoc_options = ['--charset=UTF-8']
  s.required_ruby_version = Gem::Requirement.new('>= 3.0.0')
  s.rubygems_version = '3.5.3'
  s.summary = 'RSpec for Rails'

  s.installed_by_version = '3.5.3' if s.respond_to? :installed_by_version

  s.specification_version = 4

  s.add_runtime_dependency('actionpack', ['>= 7.2'])
  s.add_runtime_dependency('activesupport', ['>= 7.2'])
  s.add_runtime_dependency('railties', ['>= 7.2'])
  s.add_runtime_dependency('rspec-core', ['~> 3.13'])
  s.add_runtime_dependency('rspec-expectations', ['~> 3.13'])
  s.add_runtime_dependency('rspec-mocks', ['~> 3.13'])
  s.add_runtime_dependency('rspec-support', ['~> 3.13'])
  s.add_development_dependency('ammeter', ['~> 1.1.5'])
  s.add_development_dependency('aruba', ['~> 2.3.1'])
  s.add_development_dependency('cucumber', ['>= 10.0'])
end
